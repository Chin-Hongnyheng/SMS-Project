...
const MODULE_OPTIONS = ['Module 1', 'Module 2', 'Module 3', 'Module 4', 'Module 5']
const COURSE_NAMES = [
...
const courseClasses = computed(() =>
  classes.value.filter((item) => !selectedCourseId.value || item.courseId === selectedCourseId.value),
)
...
const classLabel = computed(() => {
  const courseName = currentClass.value?.courseName ?? selectedCourse.value?.name ?? 'Course'
...
const selectedClassLabel = computed(
  () =>
    classOptions.value.find((option) => option.id === selectedClassId.value)?.label ??
...
const syncSelection = () => {
  if (courses.value.length > 0) {
    const hasCourse = courses.value.some((item) => item.id === selectedCourseId.value)
    if (!hasCourse) {
  ...
  } else if (!selectedClassId.value || !options.some((item) => item.id === selectedClassId.value)) {
    selectedClassId.value = options[0].id
  }
}

const fetchCourses = async () => {
  const staticCourses = COURSE_NAMES.map((name, index) => ({ id: index + 1, name }))
...
const handleToggleAttendance = async (payload: {
  ...
  if (!payload.present) {
    student.presentDays = student.presentDays.filter((day) => day !== payload.day)
  }
}
      ...
      <div class="modal" @click.stop>
        ...
        <div class="modal-body">
          <div class="modal-meta">
            <p><strong>Course:</strong> {{ selectedCourse?.name || 'Select a course' }}</p>
            <p><strong>Year:</strong> {{ selectedYear }}</p>
            <p><strong>Module:</strong> {{ formatModuleLabel(selectedModule) }}</p>
          </div>
