...
const canCreateClass = computed(
  () =>
    Boolean(classCourseId.value) &&
    Boolean(classYear.value) &&
    classModule.value.trim() !== "",
);
const formatModuleLabel = (value: string) =>
  value.replace(/^module\s*/i, "Class ");
const courseList = computed(() => courses.value.slice(0, 5));
const courseIdSet = computed(
  () => new Set(courseList.value.map((course) => course.id)),
);
const selectedCourseName = computed(() => {
  const match = courseList.value.find(
...
const displayClasses = computed(() => {
  ...
  return classes.value.filter(
    (item) => !item.courseId || courseIdSet.value.has(item.courseId),
  );
});
const classOptions = computed(() => {
  const filtered = displayClasses.value.filter((klass) => {
    if (selectedCourseId.value && klass.courseId !== selectedCourseId.value)
      return false;
    if (selectedYear.value && klass.year !== selectedYear.value) return false;
    if (selectedModule.value && klass.module !== selectedModule.value)
      return false;
    return true;
  });
...
const fetchAttendance = async () => {
  ...
  const classQuery = selectedClassId.value
    ? `&classId=${selectedClassId.value}`
    : "";
  const response = await fetch(
    `${apiBaseUrl}/attendance?month=${monthValue}${classQuery}`,
  );
  if (!response.ok) {
...
const fetchCourses = async () => {
  ...
  if (
    ...
    !list.some((course) => course.id === selectedCourseId.value)
  ) {
...
const syncSelectionFromClasses = () => {
  ...
  let target = displayClasses.value.find(
...
const handleToggleAttendance = async (payload: {
  ...
  if (payload.present) {
    ...
    target.presentDays = target.presentDays.filter(
      (day) => day !== payload.day,
    );
  }
};
    ...
    <header class="attendance-header">
      <div class="title-block">
        ...
        <p class="section-subtitle">
          Manage student attendance and class rosters.
        </p>
      </div>
      <div class="search">
        <input
      ...
      <div class="modal" @click.stop>
        ...
        <div class="modal-actions">
          ...
          <button
            class="primary"
            type="button"
            :disabled="!canCreateClass || isClassSubmitting"
            @click="submitClass"
          >
