...
const curriculumBaseUrl = `${SMS_API_BASE_URL}/curriculum`;

const fetchSubjects = async () => {
  try {
    const response = await axios.get(curriculumBaseUrl);
...
const saveSubject = async () => {
  try {
    ...
    if (isEditing.value && currentEditingId.value) {
      await axios.patch(`${curriculumBaseUrl}/${currentEditingId.value}`, newSubject.value);
    } else {
      await axios.post(curriculumBaseUrl, newSubject.value);
    }
    closeModal();
    fetchSubjects();
...
const deleteSubject = async (id: number) => {
  if (confirm("Delete this subject forever?")) {
    try {
      await axios.delete(`${curriculumBaseUrl}/${id}`);
      fetchSubjects();
